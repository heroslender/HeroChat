package com.github.heroslender.herochat.config

import com.github.heroslender.herochat.utils.appendString
import com.hypixel.hytale.codec.builder.BuilderCodec

class MessagesConfig {
    var channelNoPermission: String = "{prefix}{#FF5555}You do not have permission to send messages in this channel."
    var channelNotFound: String = "{prefix}{#FF5555}Channel not found."
    var channelJoined: String = "{prefix}{#AAAAAA}You are now talking in {#FFFFFF}{channel}{#AAAAAA}."
    var channelDisabled: String = "{prefix}{#FF5555}You have disabled this channel. Enable it again to be able to talk here."
    var chatNoRecipients: String = "{prefix}{#FF5555}No one hears you."
    var chatCooldown: String = "{prefix}{#FF5555}Please wait before sending another message."
    var chatSpamWarning: String = "{prefix}{#FF5555}Please do not spam."
    var chatCapslockWarning: String = "{prefix}{#FF5555}Please do not abuse capslock!"
    var ignoreSelf: String = "{prefix}{#FF5555}You cannot ignore yourself."
    var ignoreSuccess: String = "{prefix}{#AAAAAA}You ignored {#FFFFFF}{player}{#AAAAAA}."
    var ignoreAlready: String = "{prefix}{#FF5555}You already ignored {#FFFFFF}{player}{#FF5555}."
    var unignoreSelf: String = "{prefix}{#FF5555}You cannot unignore yourself."
    var unignoreSuccess: String = "{prefix}{#AAAAAA}You unignored {#FFFFFF}{player}{#AAAAAA}."
    var unignoreNotIgnored: String = "{prefix}{#FF5555}{#FFFFFF}{player}{#FF5555} is not ignored."
//    var targetIgnoresYou: String = "{prefix}{#FF5555}You cannot send a message to {#FFFFFF}{player}{#FF5555} because they are ignoring you."
    var privateChatStarted: String = "{prefix}{#AAAAAA}You are now in a private conversation with {#FFFFFF}{target}{#AAAAAA}."
    var privateChatPlayerNotFound: String = "{prefix}{#FF5555}The player is not online."
    var privateChatSelf: String = "{prefix}{#FF5555}You cannot start a private conversation with yourself."
    var privateChatNotActive: String = "{prefix}{#FF5555}You are not in a private conversation."
    var spyNoPermission: String = "{prefix}{#FF5555}You do not have permission to use chat spy."
    var spyToggle: String = "{prefix}{#AAAAAA}Chat spy has been {status}{#AAAAAA}."
    var nicknameNoPermission: String = "{prefix}{#FF5555}You do not have permission to change your nickname."
    var nicknameTooLong: String = "{prefix}{#FF5555}Nickname is too long."
    var nicknameContainsSpaces: String = "{prefix}{#FF5555}Your nickname can not contain spaces."
    var nicknameSet: String = "{prefix}{#AAAAAA}Your nickname has been set to {#FFFFFF}{nickname}{#AAAAAA}."
    var nicknameReset: String = "{prefix}{#AAAAAA}Your nickname has been reset."
    var menuSuccessNotificationTitle: String = "Settings Saved"
    var menuSuccessNotificationDescription: String = "Your settings were successfully saved!"
    var menuNickname: String = "Nickname"
    var menuFocusedChannel: String = "Focused Channel"
    var menuMutedChannels: String = "Muted Channels"
    var menuMessageColor: String = "Message Color"
    var menuSpyMode: String = "Spy Mode"
    var menuSaveButton: String = "Save"
    var menuCancelButton: String = "Close"

    companion object {
        @JvmField
        val CODEC: BuilderCodec<MessagesConfig> = BuilderCodec.builder(
            MessagesConfig::class.java,
            ::MessagesConfig
        )
            .appendString(MessagesConfig::channelNoPermission)
            .appendString(MessagesConfig::channelNotFound)
            .appendString(MessagesConfig::channelJoined)
            .appendString(MessagesConfig::channelDisabled)
            .appendString(MessagesConfig::chatNoRecipients)
            .appendString(MessagesConfig::chatCooldown)
            .appendString(MessagesConfig::chatSpamWarning)
            .appendString(MessagesConfig::chatCapslockWarning)
            .appendString(MessagesConfig::ignoreSelf)
            .appendString(MessagesConfig::ignoreSuccess)
            .appendString(MessagesConfig::ignoreAlready)
            .appendString(MessagesConfig::unignoreSelf)
            .appendString(MessagesConfig::unignoreSuccess)
            .appendString(MessagesConfig::unignoreNotIgnored)
//            .appendString(MessagesConfig::targetIgnoresYou)
            .appendString(MessagesConfig::privateChatStarted)
            .appendString(MessagesConfig::privateChatPlayerNotFound)
            .appendString(MessagesConfig::privateChatSelf)
            .appendString(MessagesConfig::privateChatNotActive)
            .appendString(MessagesConfig::spyNoPermission)
            .appendString(MessagesConfig::spyToggle)
            .appendString(MessagesConfig::nicknameNoPermission)
            .appendString(MessagesConfig::nicknameTooLong)
            .appendString(MessagesConfig::nicknameContainsSpaces)
            .appendString(MessagesConfig::nicknameSet)
            .appendString(MessagesConfig::nicknameReset)
            .appendString(MessagesConfig::menuNickname)
            .appendString(MessagesConfig::menuFocusedChannel)
            .appendString(MessagesConfig::menuMutedChannels)
            .appendString(MessagesConfig::menuMessageColor)
            .appendString(MessagesConfig::menuSpyMode)
            .appendString(MessagesConfig::menuSaveButton)
            .appendString(MessagesConfig::menuCancelButton)
            .appendString(MessagesConfig::menuSuccessNotificationTitle)
            .appendString(MessagesConfig::menuSuccessNotificationDescription)
            .build()
    }
}
